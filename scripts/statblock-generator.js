...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    async function getData() {
        try {
            ...
            const response = await fetch(`${base_url}${input1.value}/${input2.value.replaceAll(' ','-')}`)
    ...
    function formatBasicInfoMON(data) {
        ...
        return `<div class="text-lg font-semibold text-white space-y-2">
                    ...
                    <p><span class="italic"></span> ${data['size']} ${data['type']} ${data['alignment']}</p>
                    <p><span class="font-bold">Armor Class:</span> ${data['armor_class'][0]['value']} (${data['armor_class'][0]['type']})</p>
                    <div class="flex text-md items-center">
                        <p><span class="font-bold">Hit Points:</span> ${data['hit_points']}</p>
    ...
    // Assuming monster_names is an object with categories as keys
    Object.entries(monster_names).forEach(([category, monsters], index, array) => {
        ...
        // TODO: Add seperator after and before tome of beasts 
        // if (index < array.length-1 || index==1){
        //     const seperator = document.createElement('option')
    ...
    // import monster_names from './myArray.js';
...
//     const monsterContainer = document.getElementById('monster-preset-container');
//     const select = document.createElement('select');
//     select.className = 'bg-gray-700 text-white border border-gray-600 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 block w-full p-2.5 rounded-md shadow appearance-none cursor-pointer';
